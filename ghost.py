...
class Ghost:
    ...
    def change_direction(self):
        ...
        for i in range(0, len(ghost_dir)):
            next_x = self.pos[0] + ghost_dir[i][0] * self.speed
            next_y = self.pos[1] + ghost_dir[i][1] * self.speed
            if not self.check_collision(pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])):
                self.direction = ghost_dir[i]
